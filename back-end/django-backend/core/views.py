...
def show_disposable_mentors(request):
    ...
    """
    print('checando os mentores disponíveis'.center(90,'-'))
    # Precisamos checar se a mentorias selecionada faz parte do setor de interesse do usuário.
    user_model = request.user
    print("usuario",user_model)
    print("Estudante",Student.objects.get(user_id=user_model))
    ...
    for ind, v in enumerate(filter_):
        ...
        description = Mentor.objects.get(id=v).description
        function = Mentor.objects.get(id=v).function

        values[ind]["name"] = f"{first_name} {last_name}"
        values[ind]["description"] = description
        values[ind]["function"] = function
...
def loginUser(request):
    ...
    print('entrando no login'.center(80,'-'))
    #isStudent = Student.objects.filter(user_id=user_model).exists()
    #isMentor = Mentor.objects.filter(user=user_model).exists()
    #isJudger = Judge.objects.filter(user_id=user_model).exists()
    
    #_user_authentication = user_authentication(request,username = username,password = password)
    _user_authentication = authenticate(username = username,password = password)
    login_field = login(request, _user_authentication,)
    user_model = request.user

    print('entrando no login222'.center(80,'-'))
    ...
    user_student = Student.objects.get(user_id = user_model)
    print(user_student)
    print(user_student)
    print(type(user_student))
    name = request.user.first_name + " " + request.user.last_name
    function = user_student.function
    description = user_student.description
    print("team".center(80,'-'))
    print(user_student.team_id)
    print(type(user_student.team_id))
    print(isinstance(user_student.team_id,Team))
    print(user_student.team_id.id)
    if isinstance(user_student.team_id,Team):
        team = Team.objects.filter(id = user_student.team_id.id)
    else:
        team = Team.objects.filter(id = user_student.team_id)
    print("comprimento")
    print(len(team))
    print(team.exists())
    print(function)
